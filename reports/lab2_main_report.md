...
- Слишком большой `L` может ухудшать wall-clock время, несмотря на меньшее число итераций.
...
Сравнить `Nonlinear CG`, `HFN`, `L-BFGS(L=10)` на реальных ML-задачах по:
- значению функции,
- времени,
...
### 4.3 Результаты
- **Рисунок 2.4-1:** `fig/exp2_4_real_tasks_regression.png`
- **Рисунок 2.4-2:** `fig/exp2_4_real_tasks_classification.png`
  - `f(x_k)` vs iteration,
  - `f(x_k)` vs time,
  - `||∇f(x_k)||^2 / ||∇f(x_0)||^2` vs time (log-scale).

### 4.4 Выводы
- Метод-лидер по числу итераций не всегда лидер по времени.
- HFN делает дорогую внутреннюю работу (CG + hess_vec), что может увеличивать wall-clock.
